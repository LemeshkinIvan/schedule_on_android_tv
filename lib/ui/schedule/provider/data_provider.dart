...
class ScheduleViewModel extends ChangeNotifier {
  ...
  Map<ModelOfTableHeaders, String> genMapOperation(List<ScheduleTable> data, int index) {
    var selected = dataForTable[index];
  ...
  void applyFilter() {
    final filteredData = initialData.where((item) {
      return item.inventoryNum
          .toString()
          .toLowerCase()
          .contains(state.enteredKeyWord.toLowerCase()) ||
          item.name.toLowerCase().contains(state.enteredKeyWord.toLowerCase());
    }).toList();
